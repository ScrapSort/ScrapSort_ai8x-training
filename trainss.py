...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    contrastive_loss = ContrastiveLoss(args.batch_size, args.device,0.5)
    for train_step, (input1, input2) in enumerate(train_loader):
        # Measure data loading time
        data_time.add(time.time() - end)
        input1, input2 = input1.to(args.device), input2.to(args.device)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1, tflogger=None):
    ...
    class_probs = []
    class_preds = []
    for validation_step, (input1, input2) in enumerate(data_loader):
        with torch.no_grad():
            input1, input2 = input1.to(args.device), input2.to(args.device)
